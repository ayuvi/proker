	...
	<div class="wrapper">
		...
		<div class="content-wrapper">
			<section class="content-header">
				<h1>Surat - SK(Surat Keputusan)</h1>
			</section>
						...
						<div class="panel panel-default">
							<div class="panel-heading">
								...
								<button class="btn btn-primary pull-right" onclick='ViewData(0)'>
									<i class='fa fa-plus'></i> Add Surat - SK
								</button>
									...
									<div class="modal-dialog modal-lg">
										<div class="modal-content">
											<div class="modal-header">
												...
												<h4 class="Form-add-bu" id="addModalLabel">Form Add Surat - SK</h4>
											</div>

											<form action="#" id="form" role="form">
												<div class="modal-body">
													<input type="hidden" id="id_surat_keputusan" name="id_surat_keputusan" value='' />
													...
													<div class="row">
														<div class="col-lg-12">
															<div class="form-group">
																...
																<input type="text" id="no_keputusan" name="no_keputusan" class="form-control" placeholder="No Surat Keputusan">
															</div>
															<div class="form-group">
																<label>Perihal</label>
															...
															<div class="form-group">
																<label>Tentang</label>
																<input type="text" id="ttg" name="ttg" class="form-control" placeholder="Alamat">
															</div>
															<div class="form-group">
																<label>Tanggal Berlaku</label>
																<input type="text" id="tgl_berlaku" name="tgl_berlaku" class="form-control" placeholder="Tanggal Terdaftar">
															</div>
															<div class="form-group">
																<label>Active</label>
	...
	<div class="modal fade" id="addModalEdit" tabindex="" role="dialog" aria-labelledby="addModalLabel" aria-hidden="true">
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
					<h4 class="Form-add-bu" id="addModalLabelEdit">Form Edit Surat - SK</h4>
				</div>

				<form action="#" id="formEdit" role="form">
					<div class="modal-body">
						<input type="hidden" id="id_surat_keputusan_" name="id_surat_keputusan_" value='' />
						...
						<div class="row">
							<div class="col-lg-12">
								<div class="form-group">
									...
									<input type="text" id="no_keputusan_" name="no_keputusan_" class="form-control" placeholder="No Surat Keputusan">
								</div>
								<div class="form-group">
									<label>Perihal</label>
									<input type="text" id="perihal_" name="perihal_" class="form-control" placeholder="Atas Nama">
								</div>
								<div class="form-group">
									<label>Tentang</label>
									<input type="text" id="ttg_" name="ttg_" class="form-control" placeholder="Tentang">
								</div>
								<div class="form-group">
									<label>Tanggal Berlaku</label>
									<input type="text" id="tgl_berlaku_" name="tgl_berlaku_" class="form-control" placeholder="Tanggal Terdaftar">
								</div>
								<div class="form-group">
									<label>Active</label>
						...
						<div class="box box-primary">
							...
							<div class="box-body">
								<div class="col-lg-12">
									...
									<div class="row">
										<form id="submit">
											<input type="hidden" id="id_surat_sk_attachment" name="id_surat_sk_attachment" value='' />
		...
		<script type='text/javascript'>
			...
			$(document).ready(function() {
				...
				$("#tgl_berlaku,#tgl_berlaku").datepicker({
					changeMonth: true,
					changeYear: true,
					dateFormat: "yy-mm-dd"
				});

				$("#tgl_berlaku,#tgl_berlaku").inputmask("yyyy-mm-dd", {
					"placeholder": "yyyy-mm-dd"
				});

				$("#tgl_berlaku_,#tgl_berlaku_").datepicker({
					changeMonth: true,
					changeYear: true,
					dateFormat: "yy-mm-dd"
				});

				$("#tgl_berlaku_,#tgl_berlaku_").inputmask("yyyy-mm-dd", {
					"placeholder": "yyyy-mm-dd"
				});
			...
			var buTable = $('#buTable').DataTable({
				...
				"serverSide": true,
				ajax: {
					url: "<?= base_url() ?>surat/ax_data_surat_sk/",
					type: 'POST',
					data: function(d) {
						return $.extend({}, d, {
			...
			var attachmentTable = $('#attachmentTable').DataTable({
				...
				"serverSide": true,
				ajax: {
					url: "<?= base_url() ?>surat/ax_data_surat_sk_attachment/",
					type: 'POST',
					data: function(d) {
						return $.extend({}, d, {

							"id_surat_keputusan": $("#id_surat_sk_attachment").val(),
			...
			$('#btnSave').on('click', function() {
				...
				if ($('#no_keputusan').val() == '') {
					...
					var url = '<?= base_url() ?>surat/ax_set_data_surat_sk';
			...
			$('#btnSaveEdit').on('click', function() {
				...
				if ($('#no_keputusan_').val() == '') {
					alertify.alert("Warning", "No. npwp tidak boleh kosong.");
				} else if ($('#perihal_').val() == '') {
					alertify.alert("Warning", "Atas Nama tidak boleh kosong.");
				} else if ($('#tgl_berlaku_').val() == '') {
					alertify.alert("Warning", "Alamat tidak boleh kosong.");
				} else if ($('#ttg_').val() == '') {
					alertify.alert("Warning", "Tanggal terdaftar tidak boleh kosong.");
				} else {
					var url = '<?= base_url() ?>surat/ax_set_data_surat_sk_edit';
			...
			function ViewData(id_surat_keputusan) {
				if (id_surat_keputusan == 0) {
					...
					$('#addModalLabel').html('Add Surat - SK(Surat Keputusan)');
					$('#form')[0].reset();
					$('#active').val('1');
					$('#addModal').modal('show');
				} else {
					var url = '<?= base_url() ?>surat/ax_get_data_by_id_surat_sk';
					var data = {
					...
					$.ajax({
						...
						$('#id_surat_keputusan_').val(data['id_surat_keputusan']);

						$('#id_surat_sk_attachment').val(data['id_surat_keputusan']);
						$('#no_keputusan_').val(data['no_keputusan']);
						$('#tgl_berlaku_').val(data['tgl_berlaku']);
						$('#perihal_').val(data['perihal']);
						$('#ttg_').val(data['ttg']);
			...
			function DeleteData(id_surat_keputusan) {
				alertify.confirm(
					'Confirmation',
					'Are you sure you want to delete this data?',
					function() {
						var url = '<?= base_url() ?>surat/ax_unset_data_surat_sk';
						var data = {
							id_surat_keputusan: id_surat_keputusan
						};
			...
			$('#submit').submit(function(e) {
				...
				if ($('#nm_Attachment_edit').val() == '') {
					...
					$.ajax({
						url: '<?php echo base_url(); ?>surat/ax_upload_data_attachment_sk',
						type: "post",
			...
			function Deletenpwp(id_surat_attachment, attachment) {
				alertify.confirm(
					...
					'Are you sure you want to delete this data?',
					function() {
						var url = '<?= base_url() ?>surat/ax_unset_data_surat_sk_attachment';
						var data = {
