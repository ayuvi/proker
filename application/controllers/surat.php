...
class surat extends CI_Controller
{
    ...
    public function ax_get_data_by_id_surat_npwp()
    {
        if ($this->session->userdata('login')) {
            $session = $this->session->userdata('login');
            $menu_kd_menu_details = "S11";  //custom by database
            ...
            if (!empty($access['id_menu_details'])) {
                ...
                if (empty($id_surat_npwp))
                    ...
                    $data = $this->model_surat->get_surat_by_id_surat_npwp($id_surat_npwp);
    ...
    public function legalitas($id_jenis_surat,$id_bu)
    {
        //jika login ada di dalam session
        if ($this->session->userdata('login')) {
            // simpan session login ke dalam var
            $session = $this->session->userdata('login');
            $menu_kd_menu_details = "S11"; // custom by database
            // ambil data model menu membawa parameeter dan masukan ke var access
            $access = $this->model_menu->selectaccess($session['id_level'], $menu_kd_menu_details);
            if (!empty($access['id_menu_details'])) {
                $data['id_user'] = $session['id_user'];
                $data['nm_user'] = $session['nm_user'];
                ...
                $this->load->view('surat/surat_legalitas', $data);
            } else {
                echo "<script>alert('Anda tidak mendapatkan access menu ini');window.surat.href='javascript:history.back(-1);'</script>";
            }
        } else {
